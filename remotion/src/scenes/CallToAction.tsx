...
export const CallToAction: React.FC<{ localFrame: number }> = ({ localFrame }) => {
  ...
  return (
    <div style={{ width, height, background: "radial-gradient(ellipse at 50% 50%, #1a1005 0%, #080503 70%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: bgOpacity, position: "relative", overflow: "hidden" }}>
      {[ring1, ring2].map((r, i) => <div key={i} style={{ position: "absolute", left: "50%", top: "50%", width: interpolate(r, [0, 1], [200, 1400]), height: interpolate(r, [0, 1], [200, 1400]), borderRadius: "50%", border: "2px solid rgba(212,168,67,0.1)", transform: "translate(-50%, -50%)", opacity: interpolate(r, [0, 0.5, 1], [0.8, 0.4, 0]) }} />)}
      <div style={{ background: "#d4a843", color: "#0a0703", fontSize: 14, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", padding: "8px 24px", borderRadius: 4, marginBottom: 30, opacity: interpolate(localFrame, [5, 20], [0, 1], { extrapolateRight: "clamp" }), boxShadow: "0 0 20px rgba(212,168,67,0.5)", fontFamily: "Arial, sans-serif" }}>● FREE ON-SITE ESTIMATES</div>
      <div style={{ textAlign: "center", marginBottom: 20 }}><AnimatedText text="Ready to transform your space?" delay={10} animationType="slide-up" style={{ fontSize: 40, fontFamily: "Arial, sans-serif", color: "#9a7a3a", fontWeight: 400 }} /></div>
      <div style={{ textAlign: "center", marginBottom: 50 }}><AnimatedText text="Let's Build It Right." delay={25} animationType="scale-in" style={{ fontSize: 82, fontWeight: 900, fontFamily: "'Arial Black', sans-serif", color: "#ffffff", lineHeight: 1, textShadow: "0 0 40px rgba(212,168,67,0.4)", display: "inline-block" }} /></div>
      ...
      <div style={{ transform: `scale(${btnScale})`, background: "linear-gradient(135deg, #d4a843, #f0c860)", borderRadius: 50, padding: "22px 70px", fontSize: 26, fontWeight: 800, fontFamily: "Arial, sans-serif", color: "#0a0703", letterSpacing: 2, textTransform: "uppercase", boxShadow: "0 0 40px rgba(212,168,67,0.6)" }}>Get a Free Estimate →</div>
